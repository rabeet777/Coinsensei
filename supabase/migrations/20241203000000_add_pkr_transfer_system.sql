...
-- RLS policies for transfers table (drop if exists to avoid conflicts)
DROP POLICY IF EXISTS "Users can view their own transfers" ON user_pkr_transfers;
DROP POLICY IF EXISTS "Users can insert their own transfers" ON user_pkr_transfers;
